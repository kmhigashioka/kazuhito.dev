...
- [ ] **Merge `redesign` into `main`.**
      ...
      Verify first: `npm test && npm run check && npm run test:e2e`, expecting 31 unit,
      0 errors, 16 e2e.
...
- [ ] **`og:image`.**
      ...
      `twitter:card`, but no image, so links unfurl bare in Slack, LinkedIn and iMessage.
...
- [ ] **Hobby card accent chips.**
      `src/pages/about.astro:6` has three flat colour squares. Fine as-is. Listed only
